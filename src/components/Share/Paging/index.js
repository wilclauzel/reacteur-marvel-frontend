...
const Paging = ({ currentPage, countPage, setRequestedPage }) => {
  ...
  const [pageLastValue, setPageLastValue] = useState(null);
  ...
  useEffect(() => {
    ...
    if (crtPage >= 6) {
      ...
      setPageLastValue(nbPages - crtPage > 2 ? nbPages : null);
    } else {
      ...
      setPageLastValue(nbPages > 7 ? nbPages : null);
    }
    ...
    <div className="paging">
      <div>
        ...
        {pageLastValue && (
          <div
            className={
              Number(currentPage) === Number(pageLastValue)
                ? "page selected"
                : "page-informative"
            }
            onClick={() => {
              Number(pageLastValue) && setRequestedPage(Number(pageLastValue));
            }}
          >
            {pageLastValue}
          </div>
